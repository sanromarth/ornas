...
pub fn start_wayland_watcher(
    ...
    loop {
        ...
        let result = app_handle.run_on_main_thread(move || {
            ...
            // Hash available targets for fast structural change detection
            let targets_hash = {
            ...
            // Hash text content if targets haven't changed structurally
...
fn read_and_dispatch(
    ...
    let has_image_target = clipboard
        .wait_for_targets()
        .map(|targets| {
            targets.iter().any(|t| {
                let name = t.name();
    ...
    if has_image_target {
        tracing::debug!(
            "Wayland clipboard: image target detected, requesting image asynchronously..."
        );
        let sender_clone = sender.clone();
    ...
    // Process standard file manager URIs
    let uris = clipboard.wait_for_uris();
    if !uris.is_empty() {
        let paths = extract_file_paths_from_uris(&uris);
        if !paths.is_empty() {
            tracing::info!(count = paths.len(), "Wayland clipboard: file URIs captured");
    ...
    // Fallback for legacy GNOME file copies
    if let Some(selection_data) =
        clipboard.wait_for_contents(&gtk::gdk::Atom::intern("x-special/gnome-copied-files"))
    {
    ...
    // Fallback to plain text capture using the pre-read fingerprint text
    if let Some(text) = text {
        if !text.trim().is_empty() {
            tracing::debug!(len = text.len(), "Wayland clipboard: text captured");
            let mut item = ClipItem::from_text(text.to_string());

            if let Some(selection_data) =
                clipboard.wait_for_contents(&gtk::gdk::Atom::intern("text/html"))
            {
                let data = selection_data.data();
                if let Ok(html) = std::str::from_utf8(&data) {
                    item.content_html = Some(html.to_string());
                }
            }

            if let Some(selection_data) =
                clipboard.wait_for_contents(&gtk::gdk::Atom::intern("text/rtf"))
            {
                let data = selection_data.data();
                if let Ok(rtf) = std::str::from_utf8(&data) {
                    item.content_rtf = Some(rtf.to_string());
                }
            }
