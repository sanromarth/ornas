...
pub async fn export_backup(
    ...
    let db = std::sync::Arc::new(crate::infrastructure::database::Database::new(
        crate::infrastructure::database::connection::open_database(&crate::infrastructure::database::connection::database_path()?)?
    ));
    ...
    backup_manager.export_backup(&PathBuf::from(path), app_handle)?;
    Ok(())
}
...
pub async fn import_backup(
    ...
    let backup_manager = crate::services::backup_manager::BackupManager::new(db, images_dir);
    backup_manager.import_backup(&PathBuf::from(path), &mode, app_handle)?;
    Ok(())
}
